...
async def cities_kb(session: AsyncSession) -> InlineKeyboardMarkup:
    ...
    kb = []
    for city in cities:
        kb.append([InlineKeyboardButton(text=city.name, callback_data=f"city_{city.id}")])
...
async def edit_cities_kb(seller_id: int, session: AsyncSession) -> InlineKeyboardMarkup:
    ...
    result = await session.execute(
        select(City).join(Product).where(Product.seller_id == seller_id).distinct()
    )
    cities = result.scalars().all()

    for city in cities:
        kb.inline_keyboard.append(
            [InlineKeyboardButton(text=city.name, callback_data=f"edit_city_{city.id}")]
        )
