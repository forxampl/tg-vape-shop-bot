...
async def proc_add_city(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if city:
        ...
        await message.answer(
            _("city_added", city_name=city_name), 
            reply_markup=admin_panel_kb(),
            parse_mode="HTML"
        )
...
async def del_city_start(cb: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not cities:
        return await cb.message.answer("Нет городов для удаления")
...
async def proc_del_city(message: Message, state: FSMContext, session: AsyncSession):
    city_name = message.text.strip()

    result = await session.execute(
        select(City).where(City.name.ilike(city_name))
    )
    city = result.scalars().first()

    if not city:
        return await message.answer(
            _("city_not_found", city_name=city_name)
        )
    ...
    products_count = await session.scalar(
        select(func.count())
        .select_from(Product)
        .where(Product.city_id == city.id)
    )

    # 🔎 Считаем заказы через продукты
    ...
    # 🚫 Если есть привязки — блокируем удаление
    ...
    await message.answer(
        _("city_deleted", city_name=city.name),
        reply_markup=admin_panel_kb(),
        parse_mode="HTML"
    )
